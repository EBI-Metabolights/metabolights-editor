...
export class SampleState {
  ...
  GetStudySamples(ctx: StateContext<SamplesStateModel>, action: Samples.Get) {
    ...
    if (sampleSheetFilename) {
      ...
      Swal.fire({
        title: 'Error', text: this.samplesService.sampleSheetMissingPopupMessage, showCancelButton: false,
    ...
    this.samplesService.getTable(action.sampleSheetFilename, action.studyId).pipe(take(1)).subscribe(
      (data) => {
        ...
        const applyPriority = (cols: string[]) => {
          ...
          move("Source Name", 2);
          move("Sample Name", 3);
          move("Characteristics[Organism]", 4);
          move("Characteristics[Organism part]", 5);
          move("Characteristics[Variant]", 6);
          move("Characteristics[Sample type]", 7);
        };
