...
export class SamplesComponent {
    ...
    addSamples(){
        let samples = this.form.get('samples').value.replace(/,/g, "\n").split("\n")
        let sRows = []
        samples.forEach( s => {
            let existingSamples = this.sampleTable.data.rows.map( r => r['Sample Name'])
	...
	addColumn(type){
        if(type == 'factor'){
            ...
            mtblsCharacteristic.category = this.getOntologyComponentValue('characteristicCategory').values[0];
            ...
            let columns = []
            let characteristicsColumn = new MTBLSColumn("Characteristics["+ mtblsCharacteristic.category.annotationValue +"]", '', protocolRefIndex);
