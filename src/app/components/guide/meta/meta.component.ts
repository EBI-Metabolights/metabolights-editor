...
export class MetaComponent implements OnInit {
  ...
  getArticleFromDOI() {
    let doi = this.manuscriptIdentifier.replace('http://dx.doi.org/','').replace(/^\s+|\s+$/g,'')
  ...
  saveMetaData(){
    ...
    if(this.isManuscriptValid()){
      // Save title 
      this.editorService.saveTitle({ 'title' : this.manuscript.title }).subscribe( res => {
        // Save Abstract
        this.editorService.saveAbstract({ 'description' : this.manuscript.abstract }).subscribe( res => {
          // Save publication
          if(this.selectedManuscriptOption == 1){
            this.editorService.savePublication(this.compilePublicationBody()).subscribe( res => {
              let authorsA = []
              this.manuscript.authorDetails.forEach( author => {
                if(author.checked){
                  this.manuscript.title = ""
  ...
  compilePublicationBody(){
    ...
    mtblPublication.status = jsonConvert.deserializeObject(JSON.parse('{"comments": [],"annotationValue": "Published","termSource": null,"termAccession": "http://www.ebi.ac.uk/efo/EFO_0001796"}'), Ontology)
  ...
  compileSubmitter(author){
    ...
    mtblPerson.address = ""
    mtblPerson.affiliation = author.affiliation ? author.affiliation : "";
    let role = jsonConvert.deserializeObject(JSON.parse('{"annotationValue":"Submitter","comments":[],"termAccession":"http://purl.obolibrary.org/obo/NCIT_C42781","termSource":{"comments":[],"description":"NCI Thesaurus OBO Edition","file":"http://purl.obolibrary.org/obo/ncit.owl","ontology_name":"NCIT","provenance_name":"NCIT","version":"18.10e"}}'), Ontology)
    mtblPerson.roles.push(role)
    return mtblPerson.toJSON()
  }
