...
export class AddAssayComponent implements OnInit, OnChanges {
            ...
            if (config && config.versions) {
                ...
                if (versionConfig) {
                    ...
                    if (!this.assayForm.get('measurementType').value) {
                        let defaultMeasurement = this.activeMeasurementTypes.find(m =>
                            m.label.toLowerCase().includes('untargeted analysis') ||
                            m.value.toLowerCase().includes('untargeted analysis')
                        );

                        if (!defaultMeasurement) {
                            defaultMeasurement = this.activeMeasurementTypes.find(m =>
                                m.label === 'Untargeted Metabolite Profiling' ||
                                m.value === 'Untargeted Metabolite Profiling'
                            );
                        }
    ...
    onAssayTypeChange(value) {
        if (value) {
            const selectedType = this.activeAssayFileTemplates.find(t => t.value === value);
            if (selectedType && selectedType.label.toUpperCase().includes('LC-MS')) {
                const untargetedOption = this.activeMeasurementTypes.find(m =>
                    m.label.toLowerCase().includes('untargeted analysis') ||
                    m.value.toLowerCase().includes('untargeted analysis')
                );
                if (untargetedOption) {
                    this.assayForm.patchValue({ measurementType: untargetedOption.value });
                }
            }
        }
