...
describe('HeaderInterceptor', () => {
  ...
  it('should do nothing if the user-token header already contains a non placeholder value', () => {
    http.get('https://www.ebi.ac.uk/metabolights/ws/', {headers: {'user_token': 'proper_token'}}).subscribe(res => {
      expect(res).toBeTruthy();
    })

    const httpRequest = httpMock.expectOne("https://www.ebi.ac.uk/metabolights/ws/");
    expect(httpRequest.request.headers.get('user_token')).toEqual('proper_token');
  })
  ...
  it('should overwrite the user-token if there is a placeholder value', () => {
    ...
    const httpRequest = httpMock.expectOne("https://www.ebi.ac.uk/metabolights/ws/studies/title/title");
